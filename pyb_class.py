...
class PyBullet:
    ...
    def __init__(
        ...
        background_color = background_color if background_color is not None else np.array([255, 255, 255])
        ...
        # Setup GUI or DIRECT mode
        if self.render_mode == "human":
            self.connection_mode = p.GUI
        elif self.render_mode == "rgb_array":
            self.connection_mode = p.GUI if renderer == "OpenGL" else p.DIRECT
        else:
            raise ValueError("Invalid 'render_mode'. Must be 'rgb_array' or 'human'.")
        ...
        # p.addUserDebugLine([0, 0, 0], [1, 0, 0], [1, 1, 1], 3)  # Optional guide
        # p.addUserDebugParameter("Ambient Light", 1, 0.1, 1.0)    # Example of adding light
    ...
    def get_link_index(self, body: str, target_link_name: str) -> int:
        """Get the index of the target link in the body.

        Args:
            body (str): The name of the robot body.
            target_link_name (str): The name of the link.

        Returns:
            int: The index of the link in the robot's URDF.
        """

        robot_id = self._bodies_idx[body]
        num_joints = self.physics_client.getNumJoints(robot_id)

        for i in range(num_joints):
            info = self.physics_client.getJointInfo(robot_id, i)
            if info[12].decode('utf-8') == target_link_name:
                return i
    ...
    def get_link_orientation(self, body: str, link: int) -> np.ndarray:
        ...
        Returns:
            np.ndarray: The rotation, as (x, y, z, ws)
        """
    ...
    def get_joint_angles(self, body: str, joints: np.ndarray[int]) -> np.ndarray[float]:
        """Get the angles of multiple joints of the body.

        Args:
            body (str): Body unique name.
            joints (np.ndarray): Array of joint indices in the body.
        ...
        angles = []
        for joint in joints:
            angle = self.physics_client.getJointState(self._bodies_idx[body], joint)[0]
            angles.append(angle)
    ...
    def create_box(
        ...
        Args:
            ...
            ghost (bool): Whether the body can collide. Defaults to False.
        ...
        if texture is not None:
            texture_path = os.path.join(os.path.dirname(__file__), 'assets\\misc', texture)
            print("Loading texture from:", texture_path)
            texture_uid = self.physics_client.loadTexture(texture_path)
            print("Texture UID:", texture_uid)
    ...
    def create_plane(self, z_offset: float) -> None:
        ...
        self.create_box(
            ...
            rgba_color=np.array([0.15, 0.15, 0.15, 0.0]),
        )
